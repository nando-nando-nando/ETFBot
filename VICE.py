...
holdingsRoot = "holdings/vice"
fileLocTemp = f"{holdingsRoot}/{today}.csv"
fileLocNew = f"{holdingsRoot}/{today}.xlsx"
fileLocOld = f"{holdingsRoot}/{yesterday}.xlsx"
imgFileLocNew = f"{holdingsRoot}/imgs/AdvisorShares_VICE_Holdings_{today}.png"
url = "https://advisorshares.com/wp-content/uploads/csv/holdings/AdvisorShares_VICE_Holdings_File.csv"
header = f'🍸🎲 The latest @AdvisorShares $VICE holdings are out 🌮🚬\n'
# header = f'🔥 Hey #MSOGang, @AdvisorShares $VICE is a big mover today 🔥🇺🇸\n'
TickerColumn = 'C'
SharesColumn = 'F'
rowStart = 1
rowModifier = 0
...
def date_and_rows(sheet, header):
    try:
        ...
        logger.critical("ERROR: Couldn't collect the date and rows from the latest VICE holdings csv.")
        logger.critical(e)
