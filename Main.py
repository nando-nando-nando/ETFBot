...
fileLocNew = f"holdings/{today}.xlsx"
fileLocOld = f"holdings/{yesterday}.xlsx"
imgFileLocNew = f"holdings/imgs/AdvisorShares_MSOS_Holdings_{today}.png"
...
wbNew = load_workbook(fileLocNew)
wbOld = load_workbook(fileLocOld)
...
for ticker in newPairs:
    if ticker in oldPairs:
        diff = (int(newPairs[ticker]) - int(oldPairs[ticker]))
        if diff != 0:
