...
today = (datetime.datetime.now() + datetime.timedelta(1)).strftime(modules.settings.common_date_format)
yesterday = (datetime.datetime.now() - datetime.timedelta(2)).strftime(modules.settings.common_date_format)
...
imgFileLocNew = f"{holdingsRoot}/imgs/ForesideAmplify_CNBS_Holdings_{yesterday}.png"
...
def date_and_rows(sheet):
    try:
        ...
        print("ERROR: Couldn't collect the date and rows from the latest CNBS holdings csv.")
        print(e)
